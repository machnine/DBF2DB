from os import path
import datetime
from pypyodbc import SQL_DBMS_NAME
from dbf import Table, DbfStatus


DTYPE_MAP = {'access' : {'C': 'TEXT', 'D':'DATE', 'M': 'TEXT', 'N':'NUMBER', 'L':'BIT'},
...
class Dbf2Db:
    ...
    def __table_def(self, field):
        ...
        if len(field_def) == 1 or ',' in field_def[1]:
                ...
                return ' '.join([f'[{field_name}]', 
                                 self.__dtype_map[field_def[0]]])
        else:
    ...
    def __table_exists(self):
        ...
        if self.__dbms_name == 'sqlite':
            cur.execute(f'SELECT name FROM sqlite_master WHERE type="table" AND name="{self.__table_name}"')
